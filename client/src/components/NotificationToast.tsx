    ...
    <div className="fixed top-4 right-4 left-4 sm:left-auto sm:w-96 z-50 pointer-events-none">
      {latestNotification && (
        <div
          ...
          className="bg-white dark:bg-surface-900 border border-surface-100 dark:border-surface-800 shadow-card rounded-2xl p-4 mb-2 pointer-events-auto cursor-pointer"
          ...
          <div className="flex items-start gap-3">
            <div className="w-8 h-8 rounded-xl gradient-bg flex items-center justify-center flex-shrink-0">
              <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                ) : latestNotification.type === 'GOAL_MILESTONE' ? (
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-surface-800">{latestNotification.title}</p>
              {latestNotification.content && (
                <p className="text-xs text-surface-500 mt-0.5 line-clamp-2">{latestNotification.content}</p>
              )}
            </div>
            <button
